...
class VectorIndex:
    ...
    def search(self, query: np.ndarray, k: int = 10) -> List[int]:
        ...
        status, results = collection.search_index(query_entities, query_type=QueryType.L2, limit=k)
        if status!= 0:
    ...
    def update_product_description(self, product_id: int, new_description: str):
        ...
        entities = collection.search_index([[None]], query_type=QueryType.L2, filter_expr=partition_filter, limit=1)
        if not entities:
